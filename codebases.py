...
class Databases_deal(object):
    ...
    def use_mysql(self,*args):
        ...
        if len(args) >2:
            #insert
            count = cs1.execute(args[0],(args[1],args[2]))
        else:
            #select
    ...
    def code_save(self,code_unsave,pertain_unsave):

        coded = self.base64_bases.encrypt(code_unsave)
        pertained = self.base64_bases.encrypt(pertain_unsave)

        sql = 'insert into encryptcode(pertain,code) values(%s,%s)'
        #写入参数，防止sql注入
        flag = self.use_mysql(sql,pertained,coded)
        # print(flag)
        if flag:
            print('success')
    ...
    def code_bringout(self,search_pertain): 
        ...
        #code
        sql = 'select pertain,code from encryptcode where pertain = %s'
        ...
        try:
            ...
            return 0
        #将值返回
        return pertain,codes
        

if __name__ == "__main__":
    test = Databases_deal() 
    # test.code_save('123456as','QQ')
